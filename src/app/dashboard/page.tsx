    ...
    try {
      ...
      if (messages.length === 0) {
        setSessions(prev => prev.map(s =>
          s.id === sessionId
            ? { ...s, title: content.substring(0, 60), updatedAt: new Date().toISOString() }
            : s
        ))
      }
    } catch (err) {
        ...
        <header className="flex items-center gap-3 px-4 py-3 border-b border-slime-900/40 bg-obsidian-950/80 backdrop-blur-sm">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className={`flex items-center gap-1 text-xs px-2.5 py-1 rounded-full
              ${level === 'beginner' ? 'bg-emerald-900/50 text-emerald-400 border border-emerald-700/30' :
                level === 'advanced' ? 'bg-purple-900/50 text-purple-400 border border-purple-700/30' :
